...
import silhouette.{ Authenticator, AuthenticatorException, Identity, LoginInfo }
...
class ReadsAuthenticationPipelineSpec(implicit ev: ExecutionEnv) extends Specification with Mockito with WaitPatience {
  ...
  trait Context extends Scope {
    ...
    /**
     * The reader to retrieve the [[Identity]] for the [[LoginInfo]] stored in the [[silhouette.Authenticator]] from
     * the persistence layer.
     */
    val identityReader = mock[(LoginInfo) => Future[Option[User]]]

    /**
     * A [[Validator]] to apply to the [[silhouette.Authenticator]].
     */
    val validator = mock[Validator].smart

    /**
     * The [[Authorization]] to apply to the [[silhouette.Authenticator]] and the [[Identity]].
     */
