...
import silhouette.http.transport.{ CookieTransportSettings, EmbedIntoCookie }

import scala.concurrent.{ ExecutionContext, Future }

/**
 * Uses a stateful approach to embed the authenticator into a cookie.
 *
 * The stateful approach stores the ID of the authenticator in the cookie and the authenticator instance itself
 * in a server side backing store. On subsequent requests the ID stored in the cookie will be mapped to the
 ...
 */
case class EmbedStatefulCookie[R](
  ...
  override def apply(authenticator: Authenticator): Future[ResponsePipeline[R]] = {
    authenticatorWriter(authenticator).flatMap { authenticator =>
      authenticatorWrites.write(authenticator).map { payload =>
        EmbedIntoCookie(payload, cookieSettings)(responsePipeline).write
      }
    }
  }
}
