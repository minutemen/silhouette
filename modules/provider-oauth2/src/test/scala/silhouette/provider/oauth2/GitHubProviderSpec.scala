...
class GitHubProviderSpec extends OAuth2ProviderSpec {
    ...
    "return the social profile" in new Context {
      ...
      profile(provider.retrieveProfile(oAuth2Info)) { p =>
        p must be equalTo CommonSocialProfile(
          ...
          avatarUri = Some(uri"https://github.com/images/error/apollonia_vanova.gif")
        )
      }
    }
  }
  ...
  trait Context extends BaseContext {
    ...
    override lazy val config = spy(
      OAuth2Config(
        authorizationUri = Some(uri"https://github.com/login/oauth/authorize"),
        accessTokenUri = uri"https://github.com/login/oauth/access_token",
