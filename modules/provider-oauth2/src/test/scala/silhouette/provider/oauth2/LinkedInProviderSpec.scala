  ...
  "The `retrieveProfile` method" should {
    "fail with ProfileRetrievalException if API returns error" in new Context {
      ...
      sttpBackend = AsyncHttpClientCatsBackend.stub
        .whenRequestMatches(requestMatcher(DefaultApiUri.withParam("oauth2_access_token", oAuth2Info.accessToken)))
    ...
    "use the overridden API URI" in new Context {
      ...
      sttpBackend = AsyncHttpClientCatsBackend.stub
        .whenRequestMatches(requestMatcher(uri.withParam("oauth2_access_token", oAuth2Info.accessToken)))
