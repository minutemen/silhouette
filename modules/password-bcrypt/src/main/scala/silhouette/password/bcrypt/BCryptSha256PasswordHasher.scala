...
import silhouette.password.bcrypt.BCryptSha256PasswordHasher._
 ...
 * @see https://crypto.stackexchange.com/questions/24993/is-there-a-way-to-use-bcrypt-with-passwords-longer-than-72-
 *      bytes-securely
 */
class BCryptSha256PasswordHasher(logRounds: Int = 10) extends BCryptPasswordHasher(logRounds) {
  ...
  override def hash(plainPassword: String): PasswordInfo =
    PasswordInfo(
      hasher = id,
      password = BCrypt.hashpw(Hash.sha256(plainPassword), BCrypt.gensalt(logRounds))
    )
  ...
  override def matches(passwordInfo: PasswordInfo, suppliedPassword: String): Boolean =
    BCrypt.checkpw(Hash.sha256(suppliedPassword), passwordInfo.password)
}
